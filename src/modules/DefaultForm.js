 ...
 * processing a given chunk of text
 *
 * @since 0.1.0
 */
export class DefaultForm extends Component {
  ...
  /**
   * Handles the form submit and Retrieves the found results in the
   * text from back end
   *
  ...
  handleSubmit(event) {
    ...
    Promise
      ...
      .then(() => fetch(this.props.uri, spec))
      .then(asJSON)
  ...
  render() {
    return (
      <Grid>
        ...
        <Row className="show-grid">
          <Col xs={12}>
            <ResultList value={this.state.results}/>
          </Col>
        </Row>
      </Grid>
    );
